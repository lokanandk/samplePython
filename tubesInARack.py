...
requiredArgs.add_argument('--tube_positions', metavar='FILE', type=argparse.FileType('r'), required=True, help="Name of the tab\n" +
			"delimited text file containing the positions or (x, y) coordinates of each existing tubes,\n"+
			"one in a row.\n",dest='initialPositions')
requiredArgs.add_argument('--moves', metavar='FILE', type=argparse.FileType('r'), required=True, help="Name of the tab delimited\n" +
			"text file containing the moves, one in a row. First column must consist of the initial (x, y) coordinates\n"+
			"of the tube to be moved, second column should be the (x, y) coordinates of the destination.", dest='moves')
requiredArgs.add_argument('--remove', metavar='FILE', type=argparse.FileType('r'), required=True, help="Name of the tab delimited\n"+
			"text file containing (x, y) coordinates of the tubes to be removed, one in each row." , dest='removeList')
