...
def main():
	...
	k=0
	with open('finaltubepositions.txt','w') as myfile:
		for _ in range(len(PAMnp_nonzero[0])):
			myfile.write("({0}, {1})\n" .format(PAMnp_nonzero[0][k],PAMnp_nonzero[1][k]))
		k += 1

		array3np = np.array(array3)
		k=0
		for _ in range(len(array3np)):
			myfile.write("({0}, {1})\tremoved\n" .format(array3np[k][0],array3np[k][1]))
			k=k+1
	with open('finaltubepositions.txt','r') as outfile:
		print("\n")
		print(outfile.read())
